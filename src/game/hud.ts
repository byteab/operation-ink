...
export class MissionHUD {
  ...
  private buildMap(world: MissionWorld) {
    ...
    return `<svg viewBox="0 0 460 260" role="img" aria-label="Field map: north is up. Rail route via water tower; service route via warehouse and workshop. East annex holds underground detention, security, jeep and exit gate."><rect x="5" y="5" width="450" height="250" rx="2" fill="#edf1e9" stroke="#aab8a9"/><path d="M18 32V15l-4 7m4-7 4 7" fill="none" stroke="#365947"/><text x="16" y="45">N</text><path d="M${point(26,-32)} L${point(165,-32)}" stroke="#879587" stroke-width="4"/><path d="M${point(-53,-51)} L${point(-48.665,-51)} L${point(-34,-51)} L${point(-34,-34)} L${point(-20,-29)} L${point(17,-28)} L${point(25,-34.2)} L${point(97,-34.2)} L${point(103,-34.2)} L${point(107,-35)} L${point(142,-35)}" stroke="#365947" stroke-width="1.4" fill="none"/><path d="M${point(-40,-62.3)} L${point(-53,-62.3)} L${point(-61.8,-52)} L${point(-61.8,-44)} L${point(-53,-44)} L${point(-50,-30)} L${point(-50,4)} L${point(-20,4)} L${point(-20,20.1)} L${point(-11.75,20.1)} L${point(-11.75,14)} L${point(0,13)} L${point(55,16)} L${point(99,11)} L${point(110,5)} L${point(117,-2)} L${point(117,-9)}" stroke="#365947" stroke-dasharray="4 3" stroke-width="1.4" fill="none"/>${buildings.map(([bx,bz,w,d]) => `<rect x="${x(bx-w/2)}" y="${z(bz-d/2)}" width="${w*1.55}" height="${d*1.55}" fill="#d1dacc" stroke="#7c8f7d"/>`).join('')}<text x="${x(-43)}" y="${z(-59)}">Mess hall</text><text x="${x(-92)}" y="${z(-50)}">Service gate</text><text x="${x(7)}" y="${z(5)}">Warehouse</text><text x="${x(64)}" y="${z(3)}">Workshop</text><text x="${x(132)}" y="${z(15)}">Barracks</text><text x="${x(108)}" y="${z(-18)}">Detention</text>${world.stations.filter(s => ['cameras', 'gate', 'jeep'].includes(s.kind)).map(s => `<circle cx="${x(s.point.x)}" cy="${z(s.point.z)}" r="2.6" fill="#365947"/><text text-anchor="${s.kind === 'gate' ? 'end' : 'start'}" x="${x(s.point.x)+(s.kind === 'gate' ? -5 : 5)}" y="${z(s.point.z)-5}">${stationNames[s.kind]}</text>`).join('')}<path id="field-player" d="M0 -5 3.5 4 0 2 -3.5 4Z" fill="#984638" stroke="#fafbf9" stroke-width="1"/></svg>`
  }
