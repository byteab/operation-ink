// agent-browser eval --stdin < scripts/play-mission-inputs.js
...
(() => {
  ...
  window.__missionInputRun?.dispose();
  ...
  const loop = now => {
    ...
    if(runner.combat){
      ...
      const targets=e.mission.ai.enemies.filter(n=>n.state!=='dead'&&n.state!=='reserve'&&n.position.distanceTo(feet)<34&&e.player.world.visible(eye,n.position.clone().add(vec(0,1.15,0)),n.actor.root)&&e.player.world.visible(muzzle,n.position.clone().add(vec(0,1.15,0)),n.actor.root));
      ...
      if(targets[0] && (e.mission.weapons.current?.magazine || e.mission.weapons.current?.reserve)){
        keyUp('KeyW');keyUp('ShiftLeft');aim(targets[0].position.clone().add(vec(0,1.15,0)));
        if(!e.mission.weapons.current?.magazine)tap('KeyR');
        else if(now-lastShot>285){lastShot=now;const canvas=document.querySelector('#world');canvas.dispatchEvent(new PointerEvent('pointerdown',{button:0,bubbles:true}));window.dispatchEvent(new PointerEvent('pointerup',{button:0,bubbles:true}));}
        return;
      }
    }
    ...
    if(step.use || step.station){
      keyUp('KeyW');keyUp('ShiftLeft');
      let point;
      if(step.station){point=e.mission.world.stations.find(s=>s.kind===step.station).point;}
      ...
      aim(point);
      const action=e.player.actions.findTarget(e.camera.perspective);
      if(action && (step.station?action.kind==='mission':action.kind==='ladder')){tap('KeyF');next();}
      return;
    }
  };
  ...
  window.__missionInputRun=runner;
  return 'Input-only harness installed. Call __missionInputRun.run([...]); read status().';
})()
