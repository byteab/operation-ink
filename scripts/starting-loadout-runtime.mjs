...
const browser=(...args)=>execFileSync(process.execPath,[cli,'--session','starting-loadout',...args],{encoding:'utf8',env:{...process.env,AGENT_BROWSER_SOCKET_DIR:'/tmp/stickman-browser'},timeout:60000}).trim()
const evaluate=code=>browser('eval','-b',Buffer.from(code).toString('base64'))
const directory='docs/enemy-ai/evidence';mkdirSync(directory,{recursive:true})
